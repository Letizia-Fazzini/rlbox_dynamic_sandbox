...
def run_once(
    ...
    Prints exactly one announcement line to stderr per call.
    """
    sizes_csv = ",".join(str(s) for s in sizes)
    print(
        f"[bench] backend={backend} level={level} sizes={sizes_csv} "
...
def run_config_multi(
    ...
    pair.  Each outer run produces len(sizes) * inner_iters timing values.
    """
    num_datasets = len(sizes)
    expected_timings = num_datasets * inner_iters
    if warmup:
        run_once(cmd, cwd, backend, level, sizes, inner_iters)
    rows = []
    for i in range(iters):
        stdout = run_once(cmd, cwd, backend, level, sizes, inner_iters)
        timings = parse_timings(stdout)
        if len(timings) != expected_timings:
            raise RuntimeError(
                f"backend={backend} level={level} iter={i}: expected "
                f"{expected_timings} COMPRESSION_MS lines, got {len(timings)}"
            )
            ...
            for it in range(inner_iters):
                ...
                rows.append(
                    dict(
                        ...
                        iter=i,
                        inner_iter=it,
                        compression_ms=timings[timing_idx],
                    )
                )
    return rows
...
def main() -> int:
    ...
    ap.add_argument("--inner-iters", type=int, default=10,
                    help="inner iterations per dataset inside the binary, passed as "
                         "the third positional arg (default: 10)")
    ...
    cmd_suffix = [str(len(active_sizes)), str(inner_iters)]
    ...
    # aggregated over inner_iter and iter values.
    def key(r):
