...
COMPRESSION_RE = re.compile(r"(?:COMPRESSION_MS|SANDBOX_MS)=([\d.]+)")
...
def main() -> int:
    ...
    ap.add_argument("--shape", type=str, default="loop",
                    help="comma-separated list of dispatch shapes: "
                         "`loop` = existing per-chunk deflate loop "
                         "(main / main_process), "
                         "`stress` = one-shot deflate(Z_FINISH) over the "
                         "full input (main_stress / main_process_stress). "
                         "Pass `loop,stress` to sweep both. Meta policies "
                         "only run on `loop` (no stress variant exists).")
    args = ap.parse_args()
    ...
    def require_bin(path: Path, hint: str) -> bool:
        if path.exists():
            return True
        print(f"[bench] {path} missing; {hint}", file=sys.stderr)
        return False

    if not args.no_wasm2c:
        for sh in shapes:
            if not require_bin(
                wasm2c_build_dir / shape_bins[sh]["wasm2c"],
                f"pass --no-wasm2c or build the {sh}-shape wasm2c target",
            ):
                return 1
    ...
    if not args.no_process:
        ...
        for label, build_dir in process_builds:
            for sh in shapes:
                proc_bin = build_dir / shape_bins[sh]["process"]
                if not proc_bin.exists():
                    print(f"[bench] {proc_bin} missing; configure "
                          f"with -DRLBOX_TRANSPORT=... and build the "
                          f"{sh}-shape target", file=sys.stderr)
                    return 1
    ...
    if not args.no_wasm2c and wasm2c_build_dir not in write_dirs:
        write_dirs.append(wasm2c_build_dir)
    for _, d in process_builds:
    ...
    try:
        for size in sizes:
            ...
            for level in levels:
                ...
                print(f"[bench] size={size} level={level} (native)",
                      file=sys.stderr)
                native_pi = (write_dirs[0] / "pi.txt")
                native_rows = run_config(
                    "native",
                ...
                for shape in shapes:
                    for nr in native_rows:
                        all_rows.append({**nr, "shape": shape})

                    print(f"[bench] size={size} level={level} shape={shape}",
                          file=sys.stderr)

                    if not args.no_wasm2c:
                        wbin = wasm2c_build_dir / shape_bins[shape]["wasm2c"]
                        rows = run_config(
                            "wasm2c",
                            [str(wbin), str(level)],
                    ...
                    for label, build_dir in process_builds:
                        pbin = build_dir / shape_bins[shape]["process"]
                        rows = run_config(
                            label,
                            [str(pbin), str(level)],
                            cwd=build_dir,
                    ...
                    # Meta policies are only defined for the loop shape.
                    if shape == "loop":
    ...
    # Write CSV.
    fields = ["backend", "size_bytes", "level", "shape",
              "iter", "wall_ms", "compression_ms"]
    ...
    # Short summary: median elapsed time per (backend, size, level, shape).
    def key(r):
        return (r["backend"], r["size_bytes"], r["level"], r["shape"])

    by_key: dict = {}
    for r in all_rows:
        by_key.setdefault(key(r), []).append(r)

    print("\nbackend          size       level   shape    median_compression_ms",
          file=sys.stderr)
    print("-" * 68, file=sys.stderr)
    for k in sorted(by_key.keys()):
        rows = by_key[k]
        backend, size_bytes, level, shape = k
        t = statistics.median(r["compression_ms"] for r in rows)
        print(f"{backend:<16} {size_bytes:<10} {level:<7} {shape:<8} "
              f"{t:>10.2f}", file=sys.stderr)
